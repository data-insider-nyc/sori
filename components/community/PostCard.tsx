...
export function PostCard({ post, userId = null }: Props) {
  ...
  useEffect(() => {
    (async () => {
      const [regions, cats] = await Promise.all([getRegions(), getPostCategories()]);

      if (post.region_id) {
        const region = regions.find((r) => r.id === post.region_id);
        if (region) { setRegionValue(region.value); setRegionLabel(region.label); setRegionEmoji(region.emoji); }
      }

      const cat = cats.find((c) => c.value === post.category);
      if (cat) { setCatLabel(cat.label); setCatEmoji(cat.emoji); }
    })();
          ...
          <div className="flex items-center gap-1.5 ml-auto">
            ...
            {regionLabel && (
              <span className={cn("text-[10px] font-semibold px-2 py-0.5 rounded-full flex-shrink-0",
                getRegionColor(regionValue).bg, getRegionColor(regionValue).text)}>
                {regionEmoji} {regionLabel}
              </span>
            )}
            {/* Category badge — second */}
            <span className={cn("text-[10px] font-semibold px-2 py-0.5 rounded-full flex-shrink-0",
              getCategoryColor(post.category).bg, getCategoryColor(post.category).text)}>
              {catEmoji} {catLabel}
            </span>
