...
import { getHotTopics, getPinnedPosts } from "@/lib/queries";

export async function HotTopics() {
  const [pinned, topics] = await Promise.all([getPinnedPosts(), getHotTopics()]);

  return (
    <div>
      <div className="bg-gray-50 rounded-2xl p-5 mb-4">
        <h3 className="text-sm font-semibold text-gray-700 mb-4">📌 운영진 추천글</h3>
        {pinned.length === 0 ? (
          <p className="text-xs text-gray-400 text-center py-4">추천된 글이 없습니다</p>
        ) : (
      ...
      <div className="bg-gray-50 rounded-2xl p-5">
        <h3 className="text-sm font-semibold text-gray-700 mb-4">🔥 지금 댓글 많은 글</h3>
        {topics.length === 0 ? (
