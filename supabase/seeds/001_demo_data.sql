...
-- Reset: DELETE FROM auth.users WHERE email LIKE '%@sori.fake';
-- =============================================================================
...
-- ─────────────────────────────────────────────────────────────────────────────
INSERT INTO posts (id, user_id, category, title, content, tags, like_count, comment_count, created_at) VALUES
  ...
  (p04, uid_c, 'hospital', '팰팍 소아과 추천해주실 분?',
   ...
   ARRAY['팰팍','소아과','한인병원'], 6, 3, NOW() - INTERVAL '5 days'),
  ...
  (p05, uid_f, 'hospital', '한국어 되는 정신과 알고 계신가요?',
   ...
   ARRAY['정신과','심리상담','한인병원'], 18, 5, NOW() - INTERVAL '3 days'),
  ...
  (p06, uid_b, 'hospital', '치과 검진 포트리 근처 좋은 곳 있나요?',
   ...
   ARRAY['치과','포트리','Delta Dental'], 9, 2, NOW() - INTERVAL '6 days'),
  ...
  (p07, uid_b, 'jobs', 'H1B 비자 없이 미국에서 일할 방법?',
   ...
   ARRAY['취업','H1B','OPT','비자'], 9, 3, NOW() - INTERVAL '10 days'),
  ...
  (p08, uid_g, 'jobs', '뉴욕 IT 취업 준비 현실 후기',
   ...
   ARRAY['IT취업','뉴욕','개발자','취업후기'], 24, 6, NOW() - INTERVAL '12 days'),
  ...
  (p09, uid_a, 'realestate', '포트리 vs 팰팍 어디가 더 살기 좋아요?',
   ...
   ARRAY['부동산','포트리','팰팍','이사'], 17, 5, NOW() - INTERVAL '14 days'),
  ...
  (p10, uid_d, 'realestate', '포트리 원룸/1BR 월세 시세 어떻게 되나요?',
   ...
   ARRAY['부동산','포트리','월세','이사'], 13, 4, NOW() - INTERVAL '4 days'),
  ...
  (p13, uid_f, 'kids', '뉴저지 초등학교 입학 절차 알려주세요',
   ...
   ARRAY['초등학교','입학','뉴저지','교육'], 14, 4, NOW() - INTERVAL '9 days'),
  ...
  (p14, uid_b, 'visa', '영주권 취득 후 한국 장기 체류 괜찮나요?',
   ...
   ARRAY['영주권','그린카드','비자'], 11, 3, NOW() - INTERVAL '15 days'),
  ...
  (p17, uid_d, 'classifieds', '[구매] 아이패드 프로 12.9인치 M1 구해요',
   ...
   ARRAY['중고거래','아이패드','구매'], 2, 1, NOW() - INTERVAL '1 day'),

  (p18, uid_e, 'classifieds', '[판매] 삼성 65인치 QLED TV $350',
   '이사 가면서 TV 처분합니다.\n\n- 삼성 65인치 QLED (2022년형)\n- 상태: 매우 좋음, 흠집 없음\n- 가격: $350 (원가 $1,200)\n- 포트리 직거래\n- 자차로 픽업 가능하신 분만\n\n연락 주세요!',
   ARRAY['중고거래','TV','삼성','포트리'], 6, 3, NOW() - INTERVAL '2 days')
